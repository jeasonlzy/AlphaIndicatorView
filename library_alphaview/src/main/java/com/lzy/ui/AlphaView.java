...
public class AlphaView extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        } else if (mText != null && mIconNormal != null) {
            //计算出可用绘图的区域
            int availableWidth = getMeasuredWidth() - getPaddingLeft() - getPaddingRight();
            int availableHeight = getMeasuredHeight() - getPaddingTop() - getPaddingBottom() - mTextBound.height() - padding;
            //获取图标应该绘制的大小
            int mIconWidth = Math.min(availableWidth, availableHeight);
            //计算出图标的绘制区域
            int iconLeft = getPaddingLeft() + (availableWidth - mIconWidth) / 2;
            int iconTop = getPaddingTop();
            mIconRect.set(iconLeft, iconTop, iconLeft + mIconWidth, iconTop + mIconWidth);
            //计算文字的绘图区域
            int textLeft = getPaddingLeft() + (availableWidth - mTextBound.width()) / 2;
            int textTop = mIconRect.bottom + padding;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mIconNormal != null && mIconSelected != null) {
            ...
            canvas.drawBitmap(mIconNormal, null, mIconRect, mSelectedPaint);
            mSelectedPaint.reset();
            mSelectedPaint.setAntiAlias(true);//设置抗锯齿
            mSelectedPaint.setFilterBitmap(true);//抗锯齿
            mSelectedPaint.setAlpha(alpha); //setAlpha必须放在paint的属性最后设置，否则不起作用
            canvas.drawBitmap(mIconSelected, null, mIconRect, mSelectedPaint);
        }
        if (mText != null) {
